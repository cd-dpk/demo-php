...
<?php
	...
	if(mysqli_connect_errno()){
		...
		if($result == false){
			...
			echo "<tr>";
				echo "<th>INVOICE_ID</th><th>PRODUCT_ID</th><th>ORDERDES</th><th>QUANTITY</th>";
			echo "</tr>";
			if (mysqli_num_rows($result)>0){
				while($row = mysqli_fetch_assoc($result)){
					echo  "<tr><td>".$row["invoice_id"]. "</td><td>".$row["product_id"]. "</td><td>".$row["order_des"]."</td><td>". $row["quantity"]."</td></tr>";
				}
			}
			else {
